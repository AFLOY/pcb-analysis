...
high-precision action, so it is physics-neutral within this repository: the
thermal hexahedral operator and the electrical DC sheet operator both use it
(the electrical node grid is ``(layers, rows, cols)``, so a patch is a square
of nodes on one copper layer).
"""
