...
| ~~optimizer 向け結果 adapter~~ **解決** | — | `current-field-problem/v1`を独立にparseし、層別厚さ、center Z、複素terminal、vertical segmentをsheet meshへ変換する。結果はnamed nodeへ逆写像し、scalar/phasor voltageと面内current densityを返す | `src/electrical/dice_peec/current_field_contract.py`へ実装し、異厚3層のparse、mesh、DC end-to-endを`tests/test_current_field_contract.py`で回帰する。`plane_opt_refactor`側は`CurrentFieldProblem.as_dict()`だけを渡す | 解決済み。vertical currentとJoule lossはmetricsに接続済み。`magnetic_energy_j`は、それを読むgateの導入前にresultへ追加する必要がある |
