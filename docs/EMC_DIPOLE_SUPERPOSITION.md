...
over the loop); the C++ kernel is measured, not adopted, and stays opt-in on
the `exp/` branch. The near and far fields above remain the parts of this
package where C++ pays.
